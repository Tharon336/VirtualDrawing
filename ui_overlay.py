...
class UIOverlay:
    TOOLBAR_H = 70    # height of the top toolbar bar in pixels
    SWATCH_R  = 18    # radius of each color circle in the toolbar
    SWATCH_PAD = 10   # gap between color circles
    FONT      = cv2.FONT_HERSHEY_SIMPLEX  # the font used for all text
    ...
    def __init__(self, frame_w: int, frame_h: int):
        ...
        self._flash_timer: int = 0        # counts down in frames; 0 = hidden
    ...
    def draw_lock_zone(self, img: np.ndarray, is_locked: bool):
        ...
        color  = (0, 255, 0) if is_locked else (0, 0, 255)  # green or red
        status = "LOCK: ACTIVE" if is_locked else "LOCK: OFF (FIST HERE)"

        cv2.rectangle(img, (x1, y1), (x2, y2), color, 3)  # border
